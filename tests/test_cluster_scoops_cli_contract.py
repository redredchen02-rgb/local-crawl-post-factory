"""CLI error-contract tests for ``cluster-scoops`` (lane L8).

Pins the central CLI I/O contract (cpost.core.cli) for the cluster-scoops
...
from cpost.cli import cluster_scoops
from cpost.cli.cluster_scoops import _run
...
def _seed_two_clusters(db):
    """A corroborated 3-source scoop plus one lone single-source item."""
    _seed(db, [
...
def test_happy_seeded_library_emits_json_summary(tmp_path, monkeypatch):
    db = _db(tmp_path)
    ...
    # Valid summary shape; the command uses its own default config, so we pin
    # the invariants (every row assigned, per-cluster member counts sum to
    # items) rather than a threshold-sensitive exact cluster split.
    assert set(payload) == {"clusters", "items", "by_cluster"}
    assert payload["items"] == 4                    # all 4 library rows assigned
    assert payload["clusters"] == len(payload["by_cluster"])
    assert sum(c["member_count"] for c in payload["by_cluster"]) == 4
