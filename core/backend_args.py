...
``timeout_ms`` defaults to ``browser.backend_driver.DEFAULT_TIMEOUT_MS`` (the
...
# Fallback for browser-less callers. MUST stay == browser.backend_driver.DEFAULT_TIMEOUT_MS.
CORE_DEFAULT_TIMEOUT_MS = 30000


def _default_timeout_ms() -> int:
    try:
        from browser import backend_driver  # lazy: keep core free of a module-level browser dep
    except ImportError:
