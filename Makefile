...
vendor-htmx:
	curl -L https://unpkg.com/htmx.org@1.9.12/dist/htmx.min.js -o cpost/webui/static/htmx.min.js
...
test-full:
	python3 -m pytest -q --cov=cpost --cov-report=term-missing
...
cov:
	python3 -m pytest --cov=cpost --cov-report=term-missing
